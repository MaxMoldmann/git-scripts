...
my $cmd = "uname";
my $platform;
# Use the uname tool to determine the current platform
my @cmd_list = `$cmd`;
if (not defined($cmd_list[0]))
{
  print("[WARN] Cannot find the uname tool \n" .
        "        Assuming a Windows environment for now.\n" .
        "        (Add the folder containing \'uname\' to your PATH variable. E.g. the .. Git\\usr\\bin folder)\n");
  $platform = "Windows";      
}
else
{
...
$cmd = `$git merge-base --is-ancestor $pre_sha1 $post_sha1 $TO_NULL`; # requires Git v1.8
